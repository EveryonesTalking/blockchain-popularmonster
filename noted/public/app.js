...
// Wallet variables (keeps existing behavior)
...
/* ------------------------
   STATUS
------------------------ */
function showStatus(message, success = true, timeout = 3000) {
  if (!statusEl) return;
  statusEl.textContent = message;
  statusEl.style.color = success ? getComputedStyle(document.documentElement).getPropertyValue('--gold').trim() : '#FF6B6B';
  statusEl.style.opacity = '1';
  clearTimeout(showStatus._t);
  showStatus._t = setTimeout(() => { statusEl.textContent = ''; statusEl.style.opacity = '0'; }, timeout);
}

/* ------------------------
   WALLET (keeps existing MetaMask flow)
------------------------ */
async function connectWallet() {
  if (!window.ethereum) { showStatus('MetaMask not detected', false); return; }
  try {
    const accounts = await window.ethereum.request({ method: 'eth_requestAccounts' });
    walletAddress = accounts[0];
    if (walletAddressEl) walletAddressEl.textContent = walletAddress;
    connectWalletBtn && (connectWalletBtn.textContent = 'Connected', connectWalletBtn.disabled = true, connectWalletBtn.classList.add('disabled'));
    provider = new ethers.providers.Web3Provider(window.ethereum);
    signer = provider.getSigner();
    showStatus('Wallet connected');
  } catch (err) {
    console.error(err); showStatus('Failed to connect wallet', false);
  }
}
...
async function fetchNotes() {
  try {
    ...
    console.error(err); showStatus('Error fetching notes', false);
  }
}
...
function renderNotes(filter = '') {
  ...
  filtered.forEach(note => {
    ...
    li.innerHTML = `
      <div style="flex:1; min-width:0;">
        ...
        <div class="meta" style="opacity:.9">${new Date(note.updated_at || Date.now()).toLocaleString()}</div>
      </div>
      <div style="display:flex; gap:8px; align-items:center;">
        <button class="fav-btn" data-id="${note.id}" title="Toggle Favorite">${note.favorite ? '⭐' : '☆'}</button>
      </div>
    `;
    // click to open
  ...
  document.querySelectorAll('.fav-btn').forEach(b => b.addEventListener('click', toggleFavorite));
}
...
async function toggleFavorite(e) {
  ...
  try {
    await fetch(`${apiBase}/${id}`, { method:'PUT', headers:{'Content-Type':'application/json'}, body: JSON.stringify(note) });
    showStatus(note.favorite ? 'Favorited' : 'Unfavorited');
    renderNotes(searchEl.value);
  } catch(err){ console.error(err); showStatus('Failed to update', false); }
}
...
async function loadNote(id) {
  try {
    ...
    titleEl.style.fontFamily = font; bodyEl.style.fontFamily = font;
    favoriteBtn.textContent = note.favorite ? '⭐ Favorite' : '☆ Favorite';
    renderNotes(searchEl.value);
    showStatus('Note loaded');
  } catch(err){ console.error(err); showStatus('Failed to load note', false); }
}
...
newBtn && newBtn.addEventListener('click', ()=> {
  current = null;
  titleEl.value=''; bodyEl.value=''; favoriteBtn.textContent='☆ Favorite';
  const font = fonts.Inter; fontSelector.value = font; titleEl.style.fontFamily=font; bodyEl.style.fontFamily=font;
  titleEl.focus(); renderNotes(searchEl.value);
});
...
noteForm && noteForm.addEventListener('submit', async e => {
  ...
  const payload = { title: titleEl.value.trim(), body: bodyEl.value, favorite: existing ? existing.favorite : false, font: fontSelector.value };
  try {
    const res = current ? await fetch(`${apiBase}/${current}`, { method:'PUT', headers:{'Content-Type':'application/json'}, body:JSON.stringify(payload) })
    ...
    await fetchNotes(); showStatus(current ? 'Saved' : 'Created');
  } catch(err){ console.error(err); showStatus('Error saving', false); }
});
...
deleteBtn && deleteBtn.addEventListener('click', async ()=> {
  ...
  try {
    ...
    current = null; titleEl.value=''; bodyEl.value=''; favoriteBtn.textContent='☆ Favorite';
    await fetchNotes(); showStatus('Deleted');
  } catch(err){ console.error(err); showStatus('Error deleting', false); }
});
...
favoriteBtn && favoriteBtn.addEventListener('click', async ()=>{
  ...
  const note = notes.find(n=>n.id===current); if(!note) return;
  note.favorite = !note.favorite; favoriteBtn.classList.add('favorite-btn-animate'); setTimeout(()=>favoriteBtn.classList.remove('favorite-btn-animate'),350);
  try {
    await fetch(`${apiBase}/${current}`, { method:'PUT', headers:{'Content-Type':'application/json'}, body:JSON.stringify(note) });
    favoriteBtn.textContent = note.favorite ? '⭐ Favorite' : '☆ Favorite';
    renderNotes(searchEl.value); showStatus(note.favorite ? 'Favorited' : 'Unfavorited');
  } catch(err){ console.error(err); showStatus('Failed to update', false); }
});
...
/* ------------------------
   DARK MODE
------------------------ */
if (localStorage.getItem('theme') === 'dark') { document.body.classList.add('dark'); darkToggle && (darkToggle.textContent='☀️'); }
...
/* ------------------------
   UTIL: scroll into view if needed
------------------------ */
...
/* ------------------------
   Small safety: ensure DOM ready & initial fetch
------------------------ */
