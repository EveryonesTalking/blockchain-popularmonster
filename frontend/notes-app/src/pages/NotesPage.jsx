...
import { getNotesByUser, createNote, updateNote } from "../api/notes"
...
const NotesPage = () => {
  ...
  const saveNote = async (tab, txHash = null) => {
    try {
      ...
      if (tab.isSaved && !String(tab.id).startsWith("temp-")) {
        ...
        // Mark as saved
        setTabs((prevTabs) => prevTabs.map((t) => (t.id === tab.id ? { ...t, hasUnsavedChanges: false } : t)))
      } else {
  ...
  const handleTitleChange = (e) => {
    ...
    setTabs((prevTabs) =>
      prevTabs.map((tab) =>
        tab.id === activeTabId ? { ...tab, title: newTitle || `Untitled-${tab.id}`, hasUnsavedChanges: true } : tab,
      ),
    )
  }
  ...
  const handleContentChange = (e) => {
    ...
    setTabs((prevTabs) =>
      prevTabs.map((tab) => (tab.id === activeTabId ? { ...tab, content: newContent, hasUnsavedChanges: true } : tab)),
    )
  }
  ...
  const handleImageUpload = (e) => {
    ...
    reader.onload = (event) => {
      ...
      // Insert image into current note content
      setTabs((prevTabs) =>
        prevTabs.map((tab) =>
          tab.id === activeTabId ? { ...tab, content: tab.content + imageMarkdown, hasUnsavedChanges: true } : tab,
        ),
      )

      // Update textarea content
      if (editorRef.current) {
        editorRef.current.value = editorRef.current.value + imageMarkdown
        editorRef.current.focus()
      }
  ...
  return (
    <div className="landing-page">
            ...
            {activeTab && (
              <div className="landing-text-editor-container">
                ...
                <textarea
                  ...
                  placeholder="Start typing your notes here... or upload images from the Edit menu!"
                  spellCheck="false"
                />
              </div>
            )}
      ...
      {/* Find/Replace Modal */}
      {isFindReplaceOpen && (
        <FindReplaceModal onClose={() => setIsFindReplaceOpen(false)} onFindReplace={handleFindReplace} />
      )}
